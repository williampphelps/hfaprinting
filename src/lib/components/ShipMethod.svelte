...
<RadioGroup display="grid grid-cols-1 md:grid-cols-2 gap-4" active="variant-ghost" hover="variant-soft hover:variant-ghost" background="none" border="none">
    <RadioItem name='rate' bind:group={value} value='pickup'>
        <span class="flex flex-row gap-4 p-4 text-left items-center">
            ...
            <span class={(value == 'pickup' ? 'bg-primary-500-400-token' : '') + " rounded-full border-token border-primary-800-100-token text-center w-5 h-5 text-xs text-white material-icons"}>{#if value == 'pickup'}check{/if}</span>
        </span>
    </RadioItem>
    {#each rates as rate}
    <RadioItem name='rate' bind:group={value} value={rate.object_id}>
        <span class="flex flex-row gap-4 p-4 text-left items-center">
            <span class="flex flex-col gap-4 w-full">
                <span class="flex flex-row items-center gap-4 w-full">
            ...
            <span class={(value == rate.object_id ? 'bg-primary-500-400-token' : '') + " rounded-full border-token border-primary-800-100-token text-center w-5 h-5 text-xs text-white material-icons"}>{#if value == rate.object_id}check{/if}</span>
        </span>
    </RadioItem>
    {/each}
</RadioGroup>