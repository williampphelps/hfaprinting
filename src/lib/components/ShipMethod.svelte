<script>
    ...
    export let value = { object_id: 'pickup', amount: 0, servicelevel: { name: 'free pickup' }, provider: 'Free Pickup', duration_terms: 'Customer will be notified when order has been "shipped". ' };
    export let rates;
</script>
<RadioGroup display="grid grid-cols-1 md:grid-cols-2 gap-4" active="variant-ghost" hover="variant-soft hover:variant-ghost" background="none" border="none">
    <RadioItem name='rate' bind:group={value} value={ { object_id: 'pickup', amount: 0, servicelevel: { name: 'free pickup' }, provider: 'Free Pickup', duration_terms: 'Customer will be notified when order has been "shipped". ', provider_image_200: 'https://ik.imagekit.io/szheqbces/darklogo.png' } }>
